...
- [ ] Task 1: Create orchestrator.py scaffold (AC: #1)
  - [ ] 1.1: Create `_bmad/bmm/workflows/4-implementation/sprint-runner/dashboard/orchestrator.py`
  - [ ] 1.2: Define main `Orchestrator` class with `__init__`, `start()`, `stop()` methods
  - [ ] 1.3: Add command-line argument parsing for batch_size (number or "all")
  - [ ] 1.4: Import db.py module for database operations
  - [ ] 1.5: Import aiohttp WebSocket broadcast function from server.py
...
- [ ] Task 2: Implement project context check (AC: #1, Step 0)
  ...
  - [ ] 2.6: Log context:create or context:refresh events
...
- [ ] Task 3: Implement sprint-status.yaml reading and story selection (AC: #1, Steps 1-2)
  ...
  - [ ] 3.2: Filter stories: exclude `epic-*` and `*-retrospective` keys
  - [ ] 3.3: Sort stories numerically (1-1, 1-2, 2-1, 2a-1, etc.)
  - [ ] 3.4: Find first non-done, non-blocked story
  - [ ] 3.5: Implement story pairing logic (pair from same epic prefix)
  - [ ] 3.6: Extract epic_id from story_key (everything before last dash)

- [ ] Task 4: Implement Claude CLI spawning with asyncio.subprocess (AC: #2)
  - [ ] 4.1: Create `spawn_subagent(prompt: str, prompt_file: str)` async method
  - [ ] 4.2: Use `asyncio.subprocess.create_subprocess_exec` with `claude` executable
  - [ ] 4.3: Build args: `-p`, `--output-format`, `stream-json`
  - [ ] 4.4: Set up stdin/stdout/stderr as PIPE
  - [ ] 4.5: Write prompt to stdin, then close stdin
  - [ ] 4.6: Handle process environment (inherit PATH for claude)

- [ ] Task 5: Implement NDJSON stream parsing (AC: #3)
  - [ ] 5.1: Create `parse_ndjson_stream(process)` async generator
  - [ ] 5.2: Read stdout line by line using `readline()` or async iteration
  - [ ] 5.3: Parse each JSON line with try/except for malformed lines
  - [ ] 5.4: Extract task-id events from parsed JSON (match task-taxonomy.yaml phases)
  - [ ] 5.5: Yield parsed events for caller to handle
  - [ ] 5.6: Handle stream end gracefully

- [ ] Task 6: Implement database event logging (AC: #3, #4)
  - [ ] 6.1: Call `db.create_event()` for each parsed event
  - [ ] 6.2: Include: batch_id, story_id, command_id, timestamp, task_id, status, message
  - [ ] 6.3: Call `db.update_story()` when story status changes
  - [ ] 6.4: Ensure atomic updates (use db connection as context manager)

- [ ] Task 7: Implement WebSocket event emission (AC: #3, #4)
  - [ ] 7.1: Create `emit_event(event_type: str, payload: dict)` method
  - [ ] 7.2: Broadcast to all connected WebSocket clients via server module
  - [ ] 7.3: Emit command:start, command:progress, command:end events
  - [ ] 7.4: Emit story:status events on story transitions
  - [ ] 7.5: Handle case when no WebSocket clients connected (no-op)

- [ ] Task 8: Implement sprint-status.yaml updates (AC: #4)
  - [ ] 8.1: Create `update_sprint_status(story_key: str, new_status: str)` method
  - [ ] 8.2: Read YAML, update specific key, write back (preserve comments if possible)
  - [ ] 8.3: Use ruamel.yaml for comment-preserving round-trip if available, else pyyaml
  - [ ] 8.4: Handle file locking to prevent concurrent write corruption

- [ ] Task 9: Implement main orchestration loop (AC: #1, #5, #6)
  - [ ] 9.1: Implement Step 0: project context check
  - [ ] 9.2: Implement Step 1: read sprint-status, select stories
  - [ ] 9.3: Implement Step 2: create-story + discovery (parallel via asyncio.gather)
  - [ ] 9.4: Implement Step 2b: story-review-1, background chain if critical issues
  - [ ] 9.5: Implement Step 3: create-tech-spec (conditional on tech_spec_needed)
  - [ ] 9.6: Implement Step 3b: tech-spec-review-1, background chain if critical issues
  - [ ] 9.7: Implement Step 4: dev-story + code-review loop (sequential per story)
  - [ ] 9.8: Implement Step 4c: batch-commit workflow call
  - [ ] 9.9: Implement Step 6: cycle tracking and batch completion

- [ ] Task 10: Implement batch control (AC: #5, #6)
  - [ ] 10.1: Add `batch_mode` attribute ("all" or "fixed")
  - [ ] 10.2: Add `max_cycles` and `cycles_completed` counters
  - [ ] 10.3: Implement "all" mode: loop until all stories done/blocked
  - [ ] 10.4: Implement "fixed" mode: pause when cycles_completed >= max_cycles
  - [ ] 10.5: Emit batch:start and batch:end events
  - [ ] 10.6: Handle graceful stop request (complete current command, then exit)
