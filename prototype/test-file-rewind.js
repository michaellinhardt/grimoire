...
async function main() {
  ...
  try {
    ...
    let messages = await collectMessages(
      session.sendMessage('I created a file called rewind-test.txt with content "ORIGINAL". Acknowledge this.')
    );
    ...
    messages = await collectMessages(
      session.sendMessage('Use the Write tool to change rewind-test.txt content to just "MODIFIED" (nothing else).')
    );
    console.log('Response:', getResultText(messages));
