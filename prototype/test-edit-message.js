...
async function main() {
  ...
  try {
    ...
    messages = await collectMessages(
      session.sendMessage('my name is mike')
    );
    ...
    messages = await collectMessages(
      session.sendMessage('my name is ilan')
    );
    ...
    messages = await collectMessages(
      session.sendMessage('what is my name?')
    );
    ...
    console.log('='.repeat(60));
    console.log('Original (mike):', mikeResponse.toLowerCase().includes('mike') ? 'Said mike' : 'UNEXPECTED');
    console.log('After edit (ilan):', ilanResponse.toLowerCase().includes('ilan') ? 'Said ilan' : 'UNEXPECTED');
    console.log('Name check:', nameResponse.toLowerCase().includes('ilan') ? 'ILAN - SUCCESS' :
                              nameResponse.toLowerCase().includes('mike') ? 'MIKE - FAIL' : 'UNCLEAR');
    console.log('');
    console.log('Message edit flow:',
      ilanResponse.toLowerCase().includes('ilan') && nameResponse.toLowerCase().includes('ilan')
        ? 'SUCCESS' : 'NEEDS INVESTIGATION');
